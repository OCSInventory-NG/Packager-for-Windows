...
#define ERR_OS					ERR_MIN_PRG+17				/* mauvais OS */
#define ERR_FONC_TOOL			ERR_MIN_PRG+18				/* fonctions toolhelp non trouvees */
#define ERR_INITIALISATION		ERR_MIN_PRG+19				/* initialisation impossible */
...
static  HANDLE				hFile = INVALID_HANDLE_VALUE;	/* handle sur le fichier log */
...
/* gestion des process */
/* structure pour la gestion des process */
typedef struct tagSPROCESS_ID 
{
	DWORD	ProcessID;										/* ID du process */
	DWORD	ParentProcessID;								/* ID du process parent */
	char	szExeFile[MAX_PATH + 1]; 
	BOOL	bPresent;
} SPROCESS_ID;
#define	TAB_AJOUT			6								/* nbre de process ajoute dans la liste */
static LONG					lTabMaxProcess = 0;				/* taille du tableau */
static SPROCESS_ID			*pTabProcess = NULL;			/* tableau des process */
//#define PROCESS_NULL		0
#define PROCESS_NULL		-1


#define WM_FIN_APPLIC		WM_USER+1