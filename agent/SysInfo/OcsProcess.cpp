...
BOOL CProcess::IsProcessRunningNT(LPCTSTR lpstrProcessName)
{
	...
	if ((*(FARPROC*)&lpfnEnumProcesses = GetProcAddress( hDll, _T( "EnumProcesses"))) == NULL)
	{
	...
	if ((*(FARPROC*)&lpfnEnumProcessModules = GetProcAddress( hDll, _T( "EnumProcessModules"))) == NULL)
	{
		// Tell the user that we could not find a usable 
		// PSAPI DLL.                                  
...
#ifndef _UNICODE
	if ((*(FARPROC*)&lpfnGetModuleFileNameEx = GetProcAddress( hDll, _T( "GetModuleFileNameExA"))) == NULL)
	{
		// Tell the user that we could not find a usable 
		// PSAPI DLL.                                  
...
#else
	if ((*(FARPROC*)&lpfnGetModuleFileNameEx = GetProcAddress( hDll, _T( "GetModuleFileNameExW"))) == NULL)
	{
		// Tell the user that we could not find a usable 
		// PSAPI DLL.                                  
		...
		for (lProcessCpt=0; (lProcessCpt<lProcessNumber) && (!bFound); lProcessCpt++)
		{
			...
			if (lpfnEnumProcessModules( hProcess, lphModule, cbSize, &cbNeeded))
			{
				...
				for (lCptModule=0; (lCptModule<lModuleNumber) && (!bFound); lCptModule++)
				{
					...
					csFindedModuleName.ReleaseBuffer( _tcslen( szModuleName)+1);
					csFindedModuleName.MakeLower();
