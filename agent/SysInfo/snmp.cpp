// Document modified at : Saturday, January 03, 2004 11:36:24 AM , by user : Didier LIROULET , from computer : SNOOPY-XP-PRO
...
BOOL CSNMP::GetNetworkAdapters(CNetworkAdapterList *pList)
{
    ...
    if ((m_Init = (pSnmpExtensionInit) GetProcAddress(hDll, _T( "SnmpExtensionInit"))) == NULL)
	{
		// Cannot load SNMP MIB
    ...
    if ((m_InitEx = (pSnmpExtensionInitEx) GetProcAddress(hDll, _T( "SnmpExtensionInitEx"))) == NULL)
	{
		// Cannot load SNMP MIB
    ...
    if ((m_Query = (pSnmpExtensionQuery) GetProcAddress(hDll, _T( "SnmpExtensionQuery"))) == NULL)
	{
		// Cannot load SNMP MIB
    ...
    if ((m_Trap = (pSnmpExtensionTrap) GetProcAddress(hDll, _T( "SnmpExtensionTrap"))) == NULL)
	{
		// Cannot load SNMP MIB
		FreeLibrary( hDll);
    ...
    do {
		...
		if (!ret) 
		{
                ...
                && (varBind[1].value.asnValue.address.stream != NULL)) 
			{
				...
				lpstrBuffer = csBuffer.GetBufferSetLength( varBind[3].value.asnValue.string.length);
				strncpy( lpstrBuffer, (LPCSTR) varBind[3].value.asnValue.string.stream, varBind[3].value.asnValue.string.length);
    ...
    do {
		...
		if (!ret) 
		{
                ...
                && (varBind[2].value.asnValue.address.stream != NULL)) 
			{
				...
				ipMsk = ntohl(inet_addr ( csBuffer ));
				nbRez = ipAdr & ipMsk ;
