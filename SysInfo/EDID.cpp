...
BOOL CEdid::Connect(LPCTSTR lpstrDevice)
{
	...
	if (m_bIsWin2K)
	{
		// Win 2K or higher
#ifdef UNICODE
		if ((*(FARPROC*)&lpfnSetupDiCreateDeviceInfoListEx = GetProcAddress( m_hDll, _T( "SetupDiCreateDeviceInfoListExW"))) == NULL)
#else
	...
	{
		// Win 98/Me
		if ((*(FARPROC*)&lpfnSetupDiCreateDeviceInfoList = GetProcAddress( m_hDll, _T( "SetupDiCreateDeviceInfoList"))) == NULL)
		{
			// Tell the user that we could not find a usable SetupAPI DLL.                                  
	...
	if (m_bIsWin2K)
	{
		// Win 2K or higher
#ifdef UNICODE
		if ((*(FARPROC*)&lpfnSetupDiGetClassDevsEx = GetProcAddress( m_hDll, _T( "SetupDiGetClassDevsExW"))) == NULL)
#else
	...
	else
	{
		// Win 98/Me
#ifdef UNICODE
		if ((*(FARPROC*)&lpfnSetupDiGetClassDevs = GetProcAddress( m_hDll, _T( "SetupDiGetClassDevsW"))) == NULL)
#else
	...
	if ((*(FARPROC*)&lpfnSetupDiEnumDeviceInfo = GetProcAddress( m_hDll, _T( "SetupDiEnumDeviceInfo"))) == NULL)
	{
		// Tell the user that we could not find a usable SetupAPI DLL.                                  
	...
	if ((*(FARPROC*)&lpfnSetupDiOpenDevRegKey = GetProcAddress( m_hDll, _T( "SetupDiOpenDevRegKey"))) == NULL)
	{
		// Tell the user that we could not find a usable SetupAPI DLL.                                  
	...
	if ((*(FARPROC*)&lpfnSetupDiDestroyDeviceInfoList = GetProcAddress( m_hDll, _T( "SetupDiDestroyDeviceInfoList"))) == NULL)
	{
		// Tell the user that we could not find a usable SetupAPI DLL.                                  
...
BOOL CEdid::Disconnect()
{
	...
	if ((m_hDll != NULL) && (m_hDevInfo != INVALID_HANDLE_VALUE)) {
		...
		/*
		try {
			TODO: fait planter certaines machines
			BOOL okDestroy = SetupDiDestroyDeviceInfoList( m_hDevInfo);
			if(okDestroy==0) 
				CUtils::trace("CEDID_OKDESTROY_DEVICEINFO", AfxGetApp()->m_lpCmdLine);
			else
				CUtils::trace("CEDID_NOKDESTROY_DEVICEINFO", AfxGetApp()->m_lpCmdLine);
		}
		catch( CException * pE ) {
			CUtils::trace("CEDID_DESTROY_EXCEPTION", AfxGetApp()->m_lpCmdLine);
			pE->Delete();
		}
		*/
	}
...
BOOL CEdid::GetMonitors(CMonitorList *pMyList)
{
	...
	if (m_bIsWin2K)
		hDeviceInfoSet = lpfnSetupDiGetClassDevsEx( NULL, "DISPLAY", NULL, DIGCF_PRESENT | DIGCF_ALLCLASSES, m_hDevInfo, m_csDevice.IsEmpty() ? NULL : LPCTSTR( m_csDevice), NULL);
	else
		hDeviceInfoSet = lpfnSetupDiGetClassDevs( NULL, "DISPLAY", NULL, DIGCF_PRESENT | DIGCF_ALLCLASSES);
	...
	if (m_bIsWin2K)
		hDeviceInfoSet = lpfnSetupDiGetClassDevsEx( NULL, "MONITOR", NULL, DIGCF_PRESENT | DIGCF_ALLCLASSES, m_hDevInfo, m_csDevice.IsEmpty() ? NULL : LPCTSTR( m_csDevice), NULL);
	else
		hDeviceInfoSet = lpfnSetupDiGetClassDevs( NULL, "MONITOR", NULL, DIGCF_PRESENT | DIGCF_ALLCLASSES);
    if (hDeviceInfoSet != INVALID_HANDLE_VALUE)
	{
		spDeviceInfo.cbSize = sizeof( SP_DEVINFO_DATA);
		while (lpfnSetupDiEnumDeviceInfo( hDeviceInfoSet, dwIndex, &spDeviceInfo))
